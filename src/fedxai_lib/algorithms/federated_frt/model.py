...
from fedxai_lib.algorithms.federated_frt.utils.stats_utils import compute_firing_strengths
...
class FedFRTModel:
    ...
    def __init__(self, features_names: List[str], root_node: FedFRTNode):
        ...
        self._cache_activation_force = {rule[-1].get_comp_id(): rule[-1].mean_activation_force for rule in
                                        self._antecedents}
    ...
    def set_num_samples_by_rule(self, num_samples_by_rule):
        self._num_samples_by_rule = num_samples_by_rule

    def set_rule_weights(self, rule_weights):
